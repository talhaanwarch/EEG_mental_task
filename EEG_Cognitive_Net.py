...
path = r'../input/data/data/' # use your path
...
data2pca=[]
for i in range(36):
    data1pca.append(ica.fit_transform(data1x[i].T).ravel())
for i in range(36):
    data2pca.append(ica.fit_transform(data2x[i].T).ravel())
...
data_2=[]
for i in range(36):
    data_2.append(np.reshape(data2pca[i],(sample,-1)).T)
data_1=[]
for i in range(36):
    data_1.append(np.reshape(data1pca[i],(sample,-1)).T)    
...
for p in range(1,6):
    ...
    model = Sequential()
    model.add(Conv1D(24, (6), input_shape=(sample,1),padding='same', activation='relu'))
    model.add(MaxPooling1D(pool_size=2,strides=2,padding='same'))
    model.add(Dropout(0.5))
    model.add(Conv1D(24, (6), activation='relu',padding='same'))
    model.add(MaxPooling1D(pool_size=2,strides=2,padding='same'))
    model.add(Dropout(0.25))
    model.add(Conv1D(10, (4), activation='relu',padding='same'))
    model.add(MaxPooling1D(pool_size=2,strides=2,padding='same'))
    model.add(Dropout(0.5))
    model.add(Flatten())
    ...
    fittedModel = model.fit(X_train, y_train, batch_size = 3, epochs = 50, 
